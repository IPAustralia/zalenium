...
DockerTerminate()
{
    ...
    ShutDown
    # FIXME: handle kubernetes here...
    if [ "$SEND_ANONYMOUS_USAGE_INFO" = true -a "$KUBERNETES_ENABLED" = false ]; then
        DisplayDataProcessingAgreement
...
StartUp()
{
    ...
    if [ "$REMOTE_DEBUG_PORT" = false ]; then
         REMOTE_DEBUG_JVM_PARAMS="";
    else
         REMOTE_DEBUG_JVM_PARAMS="-agentlib:jdwp=transport=dt_socket,server=y,address=${REMOTE_DEBUG_PORT},suspend=y";
    fi
    ...
    java ${REMOTE_DEBUG_JVM_PARAMS} -cp ${SELENIUM_ARTIFACT}:${ZALENIUM_ARTIFACT} org.openqa.grid.selenium.GridLauncherV3 \
    ...
    if ! timeout --foreground "30m" bash -c WaitStarterProxy; then
...
#     if ! timeout --foreground "30s" bash -c WaitStarterProxyToRegister; then
    if ! timeout --foreground "30m" bash -c WaitStarterProxyToRegister; then
...
function usage()
{
    ...
    echo -e "\t --remoteDebugPort -> enables Java Remote Debugging using the specified port. Defults'false'"
    echo ""
    echo -e "\t stop"
    echo ""
    echo -e "\t Examples:"
